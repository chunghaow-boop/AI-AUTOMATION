...
## ⚠️ STATUS CORRECTION — VERIFIED 2026-07-27
**This sandbox no longer has Playwright.** Re-tested today:
`python3 -c "import playwright"` → not installed · no chromium binary anywhere on disk ·
`cdn.playwright.dev` and `playwright.azureedge.net` → **403 at the proxy**, so
`playwright install` cannot fetch browsers either.
**Playwright is now LOCAL-ONLY** (same class as the Whisper weights). `bash setup-local.sh`
installs it on your machine. `tools/cards.py` falls back to ffmpeg when it is unavailable.

## Status (as of the earlier session): PRE-INSTALLED. Do not install.
